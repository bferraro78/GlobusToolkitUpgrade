...
# Don't make callout if we couldn't load module.
our $accounting_groups_callout = ! $@;
...
BEGIN {
	...
	$condor_check_vanilla_files = $config->get_attribute("check_vanilla_files")
			|| "no";
...
sub new {
  my $proto = shift;
	...
	my $globus_condor_conf =
			"$Globus::Core::Paths::sysconfdir/globus-condor.conf";
	...
	if (($description->jobtype() eq 'multiple') && ($description->count > 1)) {
		$self->{STDIO_MERGER} = new Globus::GRAM::StdioMerger($self->job_dir(),
				$stdout, $stderr);
	} else {
		$self->{STDIO_MERGER} = 0;
	}
...
sub submit {
	...
	my $rc;
	# Flag if the SCRATCH_DIRECTORY environment variable is set indicating likely
	# GRAM job.
	my $scratch_isset = 0;

	# Reject jobs that want streaming, if so configured.
	if ($description->streamingrequested() && $description->streamingdisabled()) {
	...
	if ($isNFSLite && !$isManagedFork) {
		...
		if ($scratch_isset) {
			# Remote_InitialDir apparently suppresses the setting of the
			# SCRATCH_DIRECTORY env variable.
			...
			# Assume that remote_initialdir is set and the submitter knows what they
			# are doing.
...
#				} @arguments) . '"');
	...
	# In the standard universe, we can validate stdin and directory because they
	# will sent to the execution host  by condor transparently.
	...
	if (!$rc) {
		return $self->respond_with_failure_extension("open: $script_filename: $!",
	...
	my $r = "KFlops >= 600000 && (FileSystemDomain == \"wren.umiacs.umd.edu\")";
#			. " || FileSystemDomain == \"lccd.umiacs.umd.edu\")";
	...
	if (scalar(@requirements) > 0) {
		$rc = print SCRIPT_FILE "Requirements = ",
				(join(" && ", @requirements) . "\n");
	}
	if (!$rc) {
		return $self->respond_with_failure_extension(
	...
	if ($isNFSLite && !$isManagedFork) {
		...
		# Only do this if we are dealing with a GRAM job that has set up a scratch
		# area otherwise we assume it is a globus-job-run or the users is using
		# remote_initialdir.
		if ( $scratch_isset ) {
			my $sdir;
			my @flist;
  ...
  if ($description->max_wall_time() ne '') {
		...
		$rc = print SCRIPT_FILE "PeriodicRemove = (JobStatus == 2) &&"
				. " ((time() - EnteredCurrentStatus) > (" . $max_wall_time
				. " * 60))\n";
		if (!$rc) {
			return $self->respond_with_failure_extension(
					"print: $script_filename: $!",
	...
	if ($universe ne 'standard') {
		...
		if (defined($shouldtransferfiles)) {
			...
			$rc = print SCRIPT_FILE "should_transfer_files = "
					. "$shouldtransferfiles\n";
			if (!$rc) {
				return $self->respond_with_failure_extension(
						"print: $script_filename: $!",
		...
		if (defined($WhenToTransferOutput)) {
			...
			$rc = print SCRIPT_FILE "WhenToTransferOutput = "
		...
		if (!$multi_output) {
			if (defined($transfer_input_files)) {
				...
				$rc = print SCRIPT_FILE "transfer_input_files = "
						. "$transfer_input_files\n";
				if (!$rc) {
					return $self->respond_with_failure_extension(
							"print: $script_filename: $!",
		...
		if (defined($transfer_output_files)) {
			...
			$rc = print SCRIPT_FILE "transfer_output_files = "
					. "$transfer_output_files\n";
			if (!$rc) {
				return $self->respond_with_failure_extension(
						"print: $script_filename: $!",
	...
	if ($universe eq 'parallel') {
		...
		for (my $i = 0; $i < $description->count(); $i++) {
			if ($multi_output) {
				...
				$rc = print SCRIPT_FILE "Output = " . $description->directory()
						. "/job$i/stdout\n";
				if (!$rc) {
					return $self->respond_with_failure_extension(
							"print: $script_filename: $!",
				...
				$rc = print SCRIPT_FILE "Error = " . $description->directory()
						. "/job$i/stderr\n";
				if (!$rc) {
					return $self->respond_with_failure_extension(
							"print: $script_filename: $!",
				...
				my $arg_counter = 0;
				foreach (@arguments) {
					# If this argument contains a comma, we need to get the right arg.
					if ($_ =~ /,/) {
...
#				if ($universe ne 'standard') {
#					# ALB -- construct a transferinputfiles directive from shared files
#					# and per-job files.
#					my $temp_input_files_string = "";
#
#					# Shared files exist in the cache.
#					my $shared_input_files_string =
#							$description->transferinputfiles();
#					if (!defined($shared_input_files_string)) {
#						$shared_input_files_string = "";
#					}
#
#					# Per-job files also exist in the cache.
#					my $per_job_input_files_string =
#							$description->transferperjobfiles();
#					if (defined($per_job_input_files_string)) {
#						# Groups of per-job files are separated with a comma; individual
#						# files within a group are separated with a colon.
...
#							s/\$\{GSBL_CONFIG_DIR\}/$scratch_dir/g;
#
#					$self->log("Adding explicitly \"transfer_input_files = "
#							. "$temp_input_files_string\"\n");
#					$rc = print SCRIPT_FILE "transfer_input_files = "
...
#					# ALB - construct a file_remaps directive from shared files and
#					# per-job files.
#					my $temp_file_remaps_string = "";
#
#					# Shared files exist in the cache.
#					my $shared_file_remaps_string =
#							$description->transferinputfiles();
...
#
#					# Per-job files also exist in the cache.
#					my $per_job_file_remaps_string =
#							$description->transferperjobfiles();
#					if (defined($per_job_file_remaps_string)) {
#						# Groups of per-job files are separated with a comma; individua
#						# files within a group are separated with a colon.
...
#				}
	...
	local(*SUBMIT_OUT);
	local(*SUBMIT_ERR);
	$pid = IPC::Open3::open3(\*SUBMIT_IN, \*SUBMIT_OUT, \*SUBMIT_ERR,
	...
	if ($status == 0) {
		...
		if ($job_id ne '') {
			...
			$job_id = join(',', map { sprintf("%03d.%03d.%03d", $job_id, $_, 0) }
...
sub poll {
	...
	if ( open(CONDOR_LOG_FILE, ('<' . $self->{condor_logfile})) ) {
		...
		$self->nfssync($description->stdout(), 0) if ($description->stdout() ne '');
		$self->nfssync($description->stderr(), 0) if ($description->stderr() ne '');
