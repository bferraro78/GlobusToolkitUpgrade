package edu.umd.grid.bio.im.impl;
...
//BEGIN PROTECT: ServiceImports
    ...
    public class IMService extends GSBLService {
	
    /**
     * Logger.
     */
    static Log log = LogFactory.getLog(IMService.class.getName());
    ...
    public IMService() throws Exception {
		...
		log.error("Runtime configuration for IM is unavailable.");
		System.exit(1);
	    }
    ...
    static {
	    ...
	    String globusLocation = (String) env.get("GLOBUS_LOCATION");
            runtimeConfig = new GSBLRuntimeConfiguration(globusLocation + "/service_configurations/IM.runtime.xml");
	    ...
	    drupalUpdateURL = GSBLUtils.getConfigElement("drupal_update_url");
            ...
            log.error("Error loading config file information for IM: " + e);
        }
    }
     ...
     */
    public boolean runService(IMArguments myBean) {
    	this.myBean = myBean;
	...
	// BEGIN PROTECT: fileSetup
	    ...
	    log.debug( "arg string after stripping: " + argumentString );
    ...
    class MonitorJobs implements Runnable {
        ...
        private IMArguments myBean = null;
	private GSBLJob job = null;
	...
	private String [] status;
		...
		// first make sure the grid is up!
		    ...
		    // set bean
		    myBean = (IMArguments)(getArguments(rwd));
		    retrieveFiles();
		} else {
