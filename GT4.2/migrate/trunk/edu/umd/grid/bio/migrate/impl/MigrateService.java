package edu.umd.grid.bio.migrate.impl;
...
import edu.umd.grid.bio.migrate.shared.MigrateParser;
    ...
    public class MigrateService extends GSBLService {
	
    /**
     * Logger.
     */
    static Log log = LogFactory.getLog(MigrateService.class.getName());
     ...
     */
    private String serviceName = "Migrate";

    /**
     * Class constructor.
     */
    public MigrateService() throws Exception {
        super("Migrate");
        try {
	    if (runtimeConfig == null) {
		log.error("Runtime configuration for Migrate is unavailable.");
		System.exit(1);
	    }
    ...
    static {
        try {
            ...
            runtimeConfig = new GSBLRuntimeConfiguration(globusLocation + "/service_configurations/Migrate.runtime.xml");
	    ...
	    update_interval = (new Integer(Integer.valueOf(update_interval_string))).intValue();
            ...
            log.error("Error loading runtime configuration or container_status_location for Migrate: " + e);
        }
    }
     ...
     */
    public boolean runService(MigrateArguments myBean) {
    	this.myBean = myBean;
	...
	// BEGIN PROTECT: fileSetup
	    ...
	    try {
		MP = new MigrateParser(myWorkingDir, myBean.getParamFile());
		    
		//remove any menu=yes entries in the param file.
		MP.menuNo();
		    
		input_files = MP.getInputFiles(true);
		output_files = MP.getOutputFiles(true);
	    }
	    catch (Exception ge) {
		log.error("Failed to create parser: " + ge);
	    }
    ...
    class MonitorJobs implements Runnable {
        ...
        private MigrateArguments myBean = null;
	private GSBLJob job = null;
	...
	private String [] status;
		...
		// first make sure the grid is up!
		    ...
		    // set bean
		    myBean = (MigrateArguments)(getArguments(rwd));
		    retrieveFiles();
		} else {
