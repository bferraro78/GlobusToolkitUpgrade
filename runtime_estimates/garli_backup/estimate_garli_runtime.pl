...
if(@ARGV != 10) { # we are expecting 10 arguments, as follows
    print "usage: perl estimate_garli_runtime.pl [unique_id] [unique_patterns] [num_taxa] [mem_used] [datatype] [ratematrix] [statefrequencies] [ratehetmodel] [numratecats] [invariantsites]\n";
    exit;
}
...
`cat garli_header $input_table garli_matrix > $input_table\_temp`;
