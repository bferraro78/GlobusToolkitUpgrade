...
class garliEstimate extends estimate{
	...
	function returnMatrixLine(){
		$ret = $this->runtime . "\t" . $this->unique_patterns[0] . "\t" . $this->num_taxa . "\t" . $this->mem_used . "\t" . $this->datatype . "\t" . $this->rate_matrix . "\t" . $this->statefrequencies . "\t" . $this->ratehetmodel . "\t" . $this->numratecats . "\t" . $this->invariantsites . "\n";
		return $ret;
	}
		...
		if($this->unique_id != -1){
			...
			if($results != false){
				while($v = mysql_fetch_array($results)){
					$this->unique_patterns[0] = $v['uniquepatterns'];
	...
	function isMulti(){
		 if(count($this->unique_patterns)==1){
		 	return false;
		 }else{
		 	return true;
		 }
	...
	function verify(){
		if(isset($this->runtime)&&isset($this->unique_patterns[0])&&isset($this->num_taxa)&&isset($this->mem_used)&&isset($this->datatype)&&isset($this->rate_matrix)&&isset($this->statefrequencies)&&isset($this->ratehetmodel)&&isset($this->numratecats)&&isset($this->invariantsites)){
			if($this->runtime > 0 && $this->unique_patterns[0]>0 && $this->num_taxa>0 && $this->mem_used > 0){
				return true;
			}else{
				return false;
			}
	...
	function save(){
		if($this->unique_id != -1){
			...
			*/
			...
			$results = mysql_query("UPDATE garli SET uniquepatterns = ".$this->unique_patterns[0].
