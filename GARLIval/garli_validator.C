...
int init_result(RESULT& result, void*& data) {
  ...
  if(resultname.find("_initial") == -1 && resultname.find("_main") == -1) {
	...
	log_messages.printf(SCHED_MSG_LOG::MSG_DEBUG,
			    "return val from NEXUSvalidator system call: %d\n", retval);
  ...
  } else { // result unit is part of a phased GARLI analysis that produces checkpoint files, so allow result to pass validation
    log_messages.printf(SCHED_MSG_LOG::MSG_DEBUG,
			"result %s should have produced checkpoint files and may skip validation\n", resultname.c_str());
  }
  delete tmp;
  return returnval;
}
