...
public class GSBLClient {
	 ...
	 *             If the client could not be initialized.
	 */
	public GSBLClient(String svcName, String EPR) throws Exception {
	 ...
	 * that we no longer need to create this working directory on the
	 * "service side". All we want to do is create a .output folder and populate
	 * it with job replicate folders.
	 */
	public boolean createWorkingDir(String info) {
		...
		try {
			if (reps > 1) {
				...
				File outputDir = new File(myWorkingDir + unique_id + "/" + unique_id
						+ ".output/");
				...
				try {
					...
					for (int i = 0; i < reps; i++) {
						tempJobDir = new File(myWorkingDir + unique_id + "/" + unique_id
								+ ".output/job" + i + "/");
						tempJobDir.mkdir();
					}
				} catch (Exception e) {
	...
	protected static Object callObjectMethod(Object object, String methodName,
		...
		try {
			...
			log.error("No such method " + methodName + " with types of " + types);
			throw ME;
		} catch (SecurityException SE) {
			log.error("Security exception search for methods: " + SE.getMessage());
			throw SE;
			...
			log.error("ERROR: InvocationTargetException calling method " + methodName
					+ ": " + ITE.getMessage());
			ITE.printStackTrace();
			throw ITE;
		}
	...
	/**
	 * Generate a JobID of the form: XXXXXXXXX.XXXXXXXXX
	 */
	...
	public String[] parseDirectory(String pathToDir, int reps) throws Exception {
		...
		if (dir.isDirectory()) {
			...
			ArrayList<String> filenames_list = new ArrayList<String>(Arrays.asList(
						filenames));
			...
			// There must be at least "reps" number of files in this directory.
			} else if (reps > filenames.length) {
				log.error("Number of input files must equal or exceed the value of --replicates!");
				throw new Exception("Number of input files must be equal to or exceed replicates!");
			}
			returnArray = new String[reps];

			for (int i = 0; i < reps; i++) {
				log.debug("returnArray[i] is: " + pathToDir + "/" + filenames[i]);
